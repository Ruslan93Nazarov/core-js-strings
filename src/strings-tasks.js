...
function removeLastOccurrences(str, value) {
  ...
  if (str.includes(value)) {
    return str.slice(0, indexLast) + str.slice(indexLast + value.length);
  }
  return str;
}
...
function isPalindrome(str) {
  const deleteComma = str.replace(/[^a-zA-Z0-9]/g, '').toLowerCase();

  return deleteComma === deleteComma.split('').reverse().join('');
}
...
function encodeToRot13(str) {
  const originAlpha = 'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ';
  const codeAlpha = 'nopqrstuvwxyzabcdefghijklmNOPQRSTUVWXYZABCDEFGHIJKLM';
  return str.replace(
    /[a-z]/gi,
    (letter) => codeAlpha[originAlpha.indexOf(letter)]
  );
}
